   ...
   is_moving_(false)
{
  ...
  {
    wheel_separation_ = priv_node_handle_.param<double>("mobilThank you for re-upload the issue.e_robot_config/seperation_between_wheels", 0.0);
...
//  V = r * w = r * (RPM * 0.1ros::TimerEvent&0472) (Change rad/sec to RPM)
                                                      ...
                                                      dynamixel_workbench_msgs::DynamixelCommand::Response &res)
{
  ...
  //id = id, value = angle, addr_name = mode
  uint8_t id = req.id;
  std::string item_name = req.addr_name;
  //add_name: 'Goal_Position'
...
void mani_callback(const dynamixel_workbench_msgs::Javis_msgs &msg){
  for(int i = 0 ; i < 6 ; i++){
    angle[i] = msg.angle[i];
  }
}
...
int main(int argc, char **argv)
{
  ...
  //ros::Timer mani_timer = nh.createTimer(ros::Duration(0.01), &mani_callback);
  ros::Subscriber sub = nh.subscribe("manipulator", 1000, &mani_callback);
